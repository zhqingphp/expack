...
trait Table {
    /**
     * 通过表单代码生成数据
     * 使用 self::tabData(['orderStatus' => 2], static::tableToCode($tableData ?? [], ['id'])['data']);
     * @param array $data 传入的数据
     * @param array $code 表单信息(通过表单信息生成相关array code代码)
     * @param array $arr 生成写入数据信息
     * @return array
     */
    public static function tabData(array $data, array $code, array $arr = []): array {
            ...
            foreach ($val as $k => $v) {
                ...
                if (!empty($content) || static::getStrArr($v, 'nullable') == 'no') {
                    if ($key == 'decimal') {
                        ...
                        $content = (is_array($content) ? static::json($content) : $content);
                    } else if ($key == 'date') {
     ...
     * @param array $delete 排除字段
     * @param array $config 配置['字段类型'=>'组名']
     * @param array $arr
     * @param int $i
     * @return array
     */
    public function tableToCode(string $table, array $delete = ['id'], array $config = [], array $arr = [], int $i = 0): array {
        $conf = [
        ...
        foreach ($data as $k => $v) {
            if (empty($delete) || empty(in_array($k, $delete))) {
                ...
                $arr[($config[$type] ?? 'string')][$k] = $array;
            }
        }
        $content['count'] = $i;
        $content['data'] = $arr;
