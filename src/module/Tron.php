...
class Tron {
     ...
     * @return string
     */
    public function publicKeyToHex(string $publicKey): string {
    ...
    public function getWallet(bool $isMnemonic = false): array {
        ...
        $data['hex'] = $this->publicKeyToHex($data['public']);
    ...
    public function TransactionShow(string $hash): array {
        ...
        $array['amount'] = self::toAmount($money);
    ...
    public function getAllTransaction($address, int $start = 0, int $limit = 50, string $count = 'true', string $sort = '-timestamp'): array {
        $url = 'https://apiasia.tronscan.io:5566';
        $url = trim($url, '/') . "/api/transaction?sort={$sort}&count={$count}&limit={$limit}&start={$start}&address=" . $address;
        ...
        if (!empty($data = $self['data'])) {
            ...
            foreach ($data as $v) {
                ...
                if ($contractType == 31) {
                    ...
                    $arr['amount'] = self::toAmount(base_convert(substr($amount, -64), 16, 10));
                } else {
    ...
    public function getTrc20Balance(string $address): string {
        ...
        return self::toAmount(base_convert(Frame::getStrArr($body, 'constant_result.0'), 16, 10));
    }
    ...
    public function getTrxBalance(string $address): string {
        return self::toAmount($this->tron->getBalance($address));
    }
